...
// Turns a raw trail match (from the API or the local fallback) into the
// display strings the itinerary card and map legend need.
export function formatTrail(trail, units = 'mi') {
  ...
  const gap = useKm ? `${gapKmRaw.toFixed(1)} km` : `${(gapKmRaw * 0.621371).toFixed(1)} mi`;
  ...
  const palette = DIFFICULTY_PALETTE[trail.difficulty] || DIFFICULTY_PALETTE.Moderate;
